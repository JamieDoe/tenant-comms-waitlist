...
const AUDIT_ENTRIES = [
  { id: "a1", action: "Response sent to Sarah Chen", detail: "Boiler repair acknowledged", time: "14:32:07", icon: CheckCircle2, iconClass: "text-chart-1" },
  { id: "a2", action: "Maintenance ticket created", detail: "Boiler repair \u2014 14 Maple Drive", time: "14:32:08", icon: Wrench, iconClass: "text-chart-5" },
  { id: "a3", action: "Message categorised by AI", detail: "Urgent \u00b7 Maintenance \u00b7 Boiler", time: "14:30:14", icon: Shield, iconClass: "text-chart-2" },
  { id: "a4", action: "New message received", detail: "Sarah Chen \u2014 14 Maple Drive, Bristol", time: "14:30:12", icon: Inbox, iconClass: "text-chart-3" },
  { id: "a5", action: "Contractor assigned", detail: "Damp investigation \u2014 9 Birch Close", time: "13:15:44", icon: Users, iconClass: "text-chart-5" },
  { id: "a6", action: "Response sent to Tom Williams", detail: "Damp report acknowledged", time: "12:48:21", icon: CheckCircle2, iconClass: "text-chart-1" },
];
...
function InboxView({ offset }: { offset: number }) {
  ...
  // Each message gets a unique instance key (offset at which it arrived)
  // so it persists as it moves down the list, enabling smooth layout animation
...
function MaintenanceView() {
  return (
    <div className="flex flex-1 flex-col overflow-hidden p-4">
      ...
      <div className="mb-3 h-px bg-border" />
      {/* Column headers */}
      <div className="mb-2 grid grid-cols-[1fr_100px_80px_70px_60px] gap-2 text-[0.55rem] font-medium text-muted-foreground">
        <span>Request</span>
        <span>Tenant</span>
        <span>Status</span>
        <span>Priority</span>
      ...
      <div className="flex-1 space-y-1 overflow-hidden">
        {MAINTENANCE_ITEMS.map((item, i) => (
          <motion.div
            ...
            className="grid grid-cols-[1fr_100px_80px_70px_60px] items-center gap-2 rounded-lg p-2 hover:bg-muted/30"
          >
            <div>
            ...
            <span className="text-[0.55rem] text-muted-foreground">{item.tenant}</span>
            <Badge variant="outline" className={`h-4 w-fit border-0 px-1.5 text-[0.48rem] ${item.statusClass}`}>{item.status}</Badge>
            <Badge variant="outline" className={`h-4 w-fit border-0 px-1.5 text-[0.48rem] ${item.priority === "Urgent" ? "bg-destructive/10 text-destructive" : "bg-muted text-muted-foreground"}`}>{item.priority}</Badge>
      ...
      >
        ...
        >
          ...
          <div className="flex flex-1 overflow-hidden">
            {/* Sidebar */}
            <div className="hidden w-[160px] shrink-0 border-r border-border bg-muted/20 p-3 md:block">
